...
{
    global $connection;
    // Prepare a query to check if the username or email exists
    $query = $connection->prepare("SELECT COUNT(*) FROM requests WHERE username = :username");
    $query->bindParam(':username', $username, PDO::PARAM_STR);
    $query->execute();
    $result = $query->fetchColumn();

    return $result > 0; // Return true if a duplicate is found
}
function checkDuplicateEmail($email) 
{
    global $connection;
    // Prepare a query to check if the username or email exists
    $query = $connection->prepare("SELECT COUNT(*) FROM requests WHERE email = :email");
...
if($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    if($data) 
    {
        ...
        {
            echo createResponse('error', 'Email already exists.');
            exit;
        }
        if (checkDuplicateUsername($username)) 
        {
            echo createResponse('error', 'Usernmae already exists.');
            exit;
        }
       ...
       // $encrypted_email = xorEncrypt($email, 'secret_key');
        ...
        saveRequest($_SERVER['REMOTE_ADDR'], $username, $encrypted_password, $email);
    } 
    else 
    {
